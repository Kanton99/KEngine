...
void mvk::vkEngine::init() {
	...
	vma::VulkanFunctions vulkanFunctions{.vkGetInstanceProcAddr = &vkGetInstanceProcAddr, .vkGetDeviceProcAddr = &vkGetDeviceProcAddr};
...
void mvk::vkEngine::draw() {
	...
	auto cmdSubmitInfo = utils::commandBufferSubmitInfo(this->m_graphicsCommandBuffer);

	auto signalInfo = utils::semaphoreSubmitInfo(vk::PipelineStageFlagBits2::eColorAttachmentOutput, this->m_renderFinishedSemaphore);
	auto waitInfo = utils::semaphoreSubmitInfo(vk::PipelineStageFlagBits2::eAllGraphics, this->m_imageAvailableSempahore);
...
void mvk::vkEngine::_allocateCommandBuffer(vk::CommandBuffer &buffer) {
	vk::CommandBufferAllocateInfo bufferAllocationInfo{.commandPool = this->m_graphicsCommandPool, .level = vk::CommandBufferLevel::ePrimary, .commandBufferCount = 1};
	...
	this->m_immediateCommandBuffer = this->m_device.allocateCommandBuffers(immediateAllocationInfo).front();
}
...
void mvk::vkEngine::_initVulkan() {
	...
	if (!SDL_Vulkan_CreateSurface(this->m_window, static_cast<VkInstance>(this->m_instance), nullptr, &surface)) {
		std::cerr << "Error generating surface\n";
	}
	this->m_surface = vk::SurfaceKHR(surface);

	this->m_deletionStack.pushFunction([=, this]() { this->m_instance.destroySurfaceKHR(this->m_surface); });
	...
	vkb::PhysicalDevice vkbPhysDevice = selector.set_minimum_version(1, 3)
											.set_surface(static_cast<VkSurfaceKHR>(this->m_surface))
											.set_required_features_12(static_cast<VkPhysicalDeviceVulkan12Features>(features_12))
	...
	this->m_graphicsQueue = vk::Queue(vkbDevice.get_queue(vkb::QueueType::graphics).value());
	this->m_graphicsQueueIndex = vkbDevice.get_queue_index(vkb::QueueType::graphics).value();
	this->m_deletionStack.pushFunction([=, this]() {
...
void mvk::vkEngine::_initCommandPool(uint32_t queueFamilyIndex) {
	vk::CommandPoolCreateInfo poolInfo{.flags = vk::CommandPoolCreateFlagBits::eResetCommandBuffer, .queueFamilyIndex = (uint32_t)queueFamilyIndex};
...
void mvk::vkEngine::_initSwapchain(uint32_t width, uint32_t height) {
	vkb::SwapchainBuilder swapchainBuilder{static_cast<VkPhysicalDevice>(this->m_physDevice), static_cast<VkDevice>(this->m_device), static_cast<VkSurfaceKHR>(this->m_surface)};
	...
	vk::ImageCreateInfo imageInfo = mvk::utils::imageCreateInfo(this->m_depthImage.format, vk::ImageUsageFlagBits::eDepthStencilAttachment, this->m_depthImage.extent);
	...
	this->m_deletionStack.pushFunction([=, this]() {
		...
		for (unsigned long i = 0; i < this->m_graphicSwapchain.imageViews.size(); i++) {
		...
		this->m_allocator.destroyImage(this->m_depthImage.image, this->m_depthImage.allocation);
	});
}
...
void mvk::vkEngine::_createDrawImage() {
	...
	vk::ImageUsageFlags drawImageFlags = vk::ImageUsageFlagBits::eTransferSrc | vk::ImageUsageFlagBits::eTransferSrc | vk::ImageUsageFlagBits::eStorage | vk::ImageUsageFlagBits::eColorAttachment;

	vk::ImageCreateInfo drawImageInfo = mvk::utils::imageCreateInfo(this->m_drawImage.format, drawImageFlags, m_drawImage.extent);
	...
	this->m_deletionStack.pushFunction([&]() {
		...
		this->m_allocator.destroyImage(this->m_drawImage.image, this->m_drawImage.allocation);
	});
}

void mvk::vkEngine::_initGraphicPipeline(std::span<vk::DescriptorSetLayout> layouts) {
	std::cout << "Initializing default pipelines\n";
	auto vertShaderCode = utils::readFile("resources/shaders/compiled/base_vertex.vert.spv");
	auto fragShaderCode = utils::readFile("resources/shaders/compiled/base_frag.frag.spv");
	...
	this->m_graphicsPipelineLayout = this->m_device.createPipelineLayout(pipelineLayoutCreateInfo);
...
void mvk::vkEngine::_initFrameBuffers() {
	this->m_graphicSwapchain.frameBuffers.resize(this->m_graphicSwapchain.imageViews.size());

	for (size_t i = 0; i < m_graphicSwapchain.imageViews.size(); i++) {
		vk::ImageView attachments[] = {m_graphicSwapchain.imageViews[i]};

		vk::FramebufferCreateInfo framebufferInfo{.renderPass = this->m_renderPass, .attachmentCount = 1, .pAttachments = attachments, .width = this->m_swapchainExtent.width, .height = this->m_swapchainExtent.height, .layers = 1};

		m_graphicSwapchain.frameBuffers[i] = this->m_device.createFramebuffer(framebufferInfo);
	}
	this->m_deletionStack.pushFunction([&]() {
...
void mvk::vkEngine::_updateUbos(mvk::UniformDescriptorObject ubo) {
	...
	ubo.proj = glm::perspectiveRH_ZO(glm::radians(45.f), (float)this->m_drawImage.extent.width / (float)this->m_drawImage.extent.height, 1000.f, 0.1f);
	ubo.proj[1][1] *= -1;
	...
	memcpy(this->m_descriptorSet.buffer.allocationInfo.pMappedData, &ubo, sizeof(ubo));
}
...
void mvk::vkEngine::_recordCommandBuffer(vk::CommandBuffer commandBuffer, uint32_t imageIndex) {
	...
	utils::transitionImage(commandBuffer, this->m_depthImage.image, vk::ImageLayout::eUndefined, vk::ImageLayout::eDepthAttachmentOptimal);
	utils::transitionImage(commandBuffer, this->m_drawImage.image, vk::ImageLayout::eUndefined, vk::ImageLayout::eColorAttachmentOptimal);
	utils::transitionImage(commandBuffer, this->m_graphicSwapchain.images[imageIndex], vk::ImageLayout::eUndefined, vk::ImageLayout::eGeneral);
	vk::RenderingAttachmentInfo depthAttachment{
		.imageView = m_depthImage.imageView, .imageLayout = vk::ImageLayout::eDepthAttachmentOptimal, .loadOp = vk::AttachmentLoadOp::eClear, .storeOp = vk::AttachmentStoreOp::eStore, .clearValue = {.depthStencil = {.depth = 0.f}}};

	std::array<float, 4> clearColor = {0.f};
	vk::RenderingAttachmentInfo colorAttachment{
		.imageView = this->m_drawImage.imageView, .imageLayout = vk::ImageLayout::eColorAttachmentOptimal, .loadOp = vk::AttachmentLoadOp::eClear, .storeOp = vk::AttachmentStoreOp::eStore, .clearValue = {.color = {clearColor}}};

	vk::RenderingInfo renderInfo{
		.renderArea = {.extent = {.width = this->m_drawImage.extent.width, .height = this->m_drawImage.extent.height}},
		.layerCount = 1,
	...
	commandBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics, this->m_graphicsPipeline);

	vk::DescriptorSet imageSet = this->m_descriptorAllocator.allocate(this->m_device, this->_singleImageDescriptorLayout);
	{
		DescriptorWriter writer;
		writer.writeImage(1, this->m_errorCheckerboardImage.imageView, this->m_defaultSamplerNearest, vk::ImageLayout::eShaderReadOnlyOptimal, vk::DescriptorType::eCombinedImageSampler);
	...
	commandBuffer.bindIndexBuffer(this->testMeshes[2]->buffers.indexBuffer.buffer, offsets[0], vk::IndexType::eUint32);

	commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, this->m_graphicsPipelineLayout, 0, this->m_descriptorSet.descriptor, nullptr);
	/*commandBuffer.draw(3, 1, 0, 0);*/

	this->m_samplePushConstants.vertexBuffer = this->testMeshes[2]->buffers.vertexBufferAddress;

	commandBuffer.pushConstants<GPUDrawPushConstants>(this->m_graphicsPipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, this->m_samplePushConstants);

	commandBuffer.drawIndexed(this->testMeshes[2]->buffers.indexCount, 1, 0, 0, 0);

	commandBuffer.endRendering();

	mvk::utils::transitionImage(this->m_graphicsCommandBuffer, this->m_drawImage.image, vk::ImageLayout::eColorAttachmentOptimal, vk::ImageLayout::eTransferSrcOptimal);
	mvk::utils::transitionImage(this->m_graphicsCommandBuffer, this->m_graphicSwapchain.images[imageIndex], vk::ImageLayout::eGeneral, vk::ImageLayout::eTransferDstOptimal);

	vk::Extent2D drawImageExtent{this->m_drawImage.extent.width, this->m_drawImage.extent.height};
	utils::copyImageToImage(commandBuffer, this->m_drawImage.image, this->m_graphicSwapchain.images[imageIndex], drawImageExtent, this->m_swapchainExtent);

	utils::transitionImage(commandBuffer, this->m_graphicSwapchain.images[imageIndex], vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::ePresentSrcKHR);
	commandBuffer.end();
}
...
void mvk::vkEngine::_initSynchronizationObjects() {
	...
	this->m_imageAvailableSempahore = this->m_device.createSemaphore(semaphore_info);
	this->m_renderFinishedSemaphore = this->m_device.createSemaphore(semaphore_info);
...
void mvk::vkEngine::_destroyBuffer(const mvk::AllocatedBuffer &buffer) { this->m_allocator.destroyBuffer(buffer.buffer, buffer.allocation); }

mvk::MeshData mvk::vkEngine::uploadMesh(std::span<mvk::VertexData> vertices, std::span<unsigned int> indeces) {
	const auto vertexDataSize = sizeof(mvk::VertexData) * vertices.size();
	const auto indecesDataSize = sizeof(unsigned int) * indeces.size();

	mvk::MeshData mesh;
	mesh.vertexBuffer = this->_allocateBuffer(vertexDataSize, vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress, vma::AllocationCreateFlagBits::eMapped,
											  vma::MemoryUsage::eGpuOnly);
	mesh.indexBuffer = this->_allocateBuffer(indecesDataSize, vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eIndexBuffer, vma::AllocationCreateFlagBits::eMapped, vma::MemoryUsage::eGpuOnly);
	...
	_immediateSubmit([&](vk::CommandBuffer cmd) {
		vk::BufferCopy vertexCopy{.srcOffset = 0, .dstOffset = 0, .size = vertexDataSize};

		cmd.copyBuffer(staginBuffer.buffer, mesh.vertexBuffer.buffer, vertexCopy);
		vk::BufferCopy indexCopy{.srcOffset = vertexDataSize, .dstOffset = 0, .size = indecesDataSize};
...
void mvk::vkEngine::_initDescriptors() {
	std::cout << "Initializing default descriptors\n";
	std::vector<DescriptoAllocatorGrowable::PoolSizeRatio> ratios = {{vk::DescriptorType::eUniformBuffer, 1}, {vk::DescriptorType::eStorageImage, 1}};
	// ratios[0].type = vk::DescriptorType::eUniformBuffer;
	// ratios[0].ratio = 1.f;
	this->m_descriptorAllocator.init(this->m_device, 10, ratios);
	...
	vk::DescriptorSetLayout mainLayout;
	{
		...
		mainLayout = builder.build(this->m_device, vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment);
	}

	{
		mvk::DescriptorLayoutBuidler builder;
		builder.addBinding(0, vk::DescriptorType::eCombinedImageSampler);
		_singleImageDescriptorLayout = builder.build(this->m_device, vk::ShaderStageFlagBits::eFragment);
	}

	this->m_descriptorSet.layout = mainLayout;
	this->m_descriptorSet.descriptor = this->m_descriptorAllocator.allocate(this->m_device, mainLayout);

	this->m_descriptorSet.buffer =
		this->_allocateBuffer(sizeof(mvk::UniformDescriptorObject), vk::BufferUsageFlagBits::eUniformBuffer, vma::AllocationCreateFlagBits::eMapped | vma::AllocationCreateFlagBits::eHostAccessSequentialWrite, vma::MemoryUsage::eAuto);
	...
	this->m_deletionStack.pushFunction([&]() {
		...
		this->m_device.destroyDescriptorSetLayout(this->_singleImageDescriptorLayout);
...
mvk::AllocatedImage mvk::vkEngine::_createImage(vk::Extent3D size, vk::Format format, vk::ImageUsageFlags usage, bool mipmapped) {
	...
	vk::ImageViewCreateInfo viewInfo = utils::imageViewCreateInfo(format, newImage.image, aspectFlags);
...
void mvk::vkEngine::_initDefaultData() {
	...
	m_whiteImage = this->_createImage((void *)&white, vk::Extent3D(1, 1, 1), vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eSampled);

	uint32_t gray = glm::packUnorm4x8(glm::vec4(0.66f, 0.66f, 0.66f, 1));
	m_greyImage = this->_createImage((void *)&gray, vk::Extent3D(1, 1, 1), vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eSampled);

	uint32_t black = glm::packUnorm4x8(glm::vec4(0, 0, 0, 1));
	m_blackImage = this->_createImage((void *)&black, vk::Extent3D(1, 1, 1), vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eSampled);
	...
	m_errorCheckerboardImage = this->_createImage(pixels.data(), vk::Extent3D(16, 16, 1), vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eSampled);

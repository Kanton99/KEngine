...
std::optional<std::vector<std::shared_ptr<mvk::MeshAsset>>> mvk::loadGltfMesh(mvk::vkEngine *engine, std::filesystem::path filePath) {
	...
	constexpr auto options = fastgltf::Options::None;
	...
	for (fastgltf::Mesh &mesh : gltf.meshes) {
		...
		for (auto &&p : mesh.primitives) {
			...
			// LOAD INDECES
			{
				...
				fastgltf::iterateAccessor<std::uint32_t>(gltf, indexAccessor, [&](std::uint32_t idx) { indeces.push_back(idx + (unsigned int)initial_vtx); });
			}
			...
			if (normals != p.attributes.end()) {
				fastgltf::iterateAccessorWithIndex<glm::vec3>(gltf, gltf.accessors[(*normals).accessorIndex], [&](glm::vec3 v, size_t index) { vertices[initial_vtx + index].normal = v; });
			}
			...
			if (colors != p.attributes.end()) {
				fastgltf::iterateAccessorWithIndex<glm::vec4>(gltf, gltf.accessors[(*colors).accessorIndex], [&](glm::vec4 v, size_t index) { vertices[initial_vtx + index].colour = v; });
			}
		...
		if (showNormals) {
			for (VertexData &vtx : vertices) {
				auto corrected = (vtx.normal + glm::vec3(1, 1, 1)) / 2.f;
				vtx.colour = glm::vec4(corrected, 1.f);
			}
		}

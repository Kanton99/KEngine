...
std::optional<std::vector<std::shared_ptr<mvk::MeshAsset>>>
mvk::loadGltfMesh(mvk::vkEngine *engine, std::filesystem::path filePath) {
  ...
  auto load =
      parser.loadGltfBinary(dataBuffer.get(), filePath.parent_path(), options);
  if (load)
    ...
    for (auto &&p : mesh.primitives) {
      ...
      // LOAD INDECES
      {
        ...
        fastgltf::iterateAccessor<std::uint32_t>(
            gltf, indexAccessor,
            [&](std::uint32_t idx) { indeces.push_back(idx + initial_vtx); });
      }
